...
export default function SignContractPage() {
  ...
  const alreadySigned = data?.status === "ditandatangani";
    ...
    <div className="max-w-2xl mx-auto p-4">
      ...
      {data && alreadySigned && (
        <div className="card p-8 text-center mt-6">
          ...
          <h1 className="text-lg font-bold mb-1">Kontrak Sudah Ditandatangani</h1>
          <p className="text-sm text-ink-muted mb-1">
            <span className="font-medium">{data.nama}</span> — {data.kategori}
          </p>
          <p className="text-sm text-ink-muted mb-5">
            Link ini sudah dipakai untuk tanda tangan sebelumnya, jadi gak bisa dipakai buat ttd ulang.
          </p>
          {data.signedUrl && (
